...
public class Simon3 : MonoBehaviour
{
    private static List<int> ordre = new List<int>();
    private static List<Simon3> listSprite = new List<Simon3>();
        ...
        if (!flag2)
        {
            if (ordreBouton[cp] == name) // c'est bon
            {
                ...
                if (cp == nbreCouleur) // victoire partielle
                {
                    ...
                    {
                        SceneManager.LoadScene("ArmurerieACA");
                    }
                    nbreCouleur++;
                    cp = 0;
                    StartCoroutine(AfficheOrdre(nbreCouleur));
            ...
            else // erreur
            {
                ...
                SceneManager.LoadScene("LaserACB");
            }
        }
    }
